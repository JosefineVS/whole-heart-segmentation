...
for i in range(len(filelist_train)):
    img, lab = create_data(filelist_train[i],'cor')
    x_train[i] = img
    y_train[i] = lab    
...
#    img, lab = create_data(filelist_val[i],'cor')
#    x_val[i] = img
#    y_val[i] = lab    
...
#        print("Validation:", (epoch + 1), '  avg_loss= ', "{:.9f}".format(np.mean(valid_loss)), '  avg_acc= ', "{:.9f}".format(np.mean(valid_accuracy)))
    ...
    saver.save(sess,"WHS/Results/segmentation/model_cor/model.ckpt")
    np.save('WHS/Results/train_hist/segmentation/train_loss_cor',train_loss)
    np.save('WHS/Results/train_hist/segmentation/train_acc_cor',train_accuracy)
#    np.save('WHS/Results/train_hist/segmentation/valid_loss_cor',valid_loss)
#    np.save('WHS/Results/train_hist/segmentation/valid_acc_cor',valid_accuracy)
